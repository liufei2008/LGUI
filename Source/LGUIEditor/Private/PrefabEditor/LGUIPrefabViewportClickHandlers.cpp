...
namespace LGUIPrefabViewportClickHandlers
{
	...
	bool ClickActor(FLGUIPrefabEditorViewportClient* ViewportClient,AActor* Actor,const FViewportClick& Click,bool bAllowSelectionChange)
	{
		...
		else if( Click.GetKey() == EKeys::RightMouseButton && !Click.IsControlDown() && !ViewportClient->Viewport->KeyState(EKeys::LeftMouseButton) )
		{
			...
			if( Actor )
			{
				...
				UE_LOG(LGUIEditor, Log,  TEXT("Clicking on Actor (context menu): %s (%s)"), *Actor->GetClass()->GetName(), *Actor->GetActorLabel() );
		...
		else if( Click.GetEvent() == IE_DoubleClick && Click.GetKey() == EKeys::LeftMouseButton && !Click.IsControlDown() && !Click.IsShiftDown() )
		{
			if( Actor )
			{
				...
				UE_LOG(LGUIEditor, Log,  TEXT("Clicking on Actor (double click): %s (%s)"), *Actor->GetClass()->GetName(), *Actor->GetActorLabel());
			...
			else if ( Actor )
			{
				if( bAllowSelectionChange  && GEditor->CanSelectActor(Actor, true, true, true) )
				{
					...
					if( Click.IsControlDown() && !bComponentSelected )
					{
						...
						if( bSelect )
						{
							UE_LOG(LGUIEditor, Log,  TEXT("Clicking on Actor (CTRL LMB): %s (%s)"), *Actor->GetClass()->GetName(), *Actor->GetActorLabel());
						}
					...
					else
					{
						...
						UE_LOG(LGUIEditor, Log,  TEXT("Clicking on Actor (LMB): %s (%s)"), *Actor->GetClass()->GetName(), *Actor->GetActorLabel());
	...
	bool ClickComponent(FLGUIPrefabEditorViewportClient* ViewportClient, HActor* ActorHitProxy, const FViewportClick& Click)
	{
		...
		else if (Click.GetKey() == EKeys::RightMouseButton && !Click.IsControlDown() && !ViewportClient->Viewport->KeyState(EKeys::LeftMouseButton))
		{
			...
			UE_LOG(LGUIEditor, Log, TEXT("Clicking on Component (context menu): %s (%s)"), *Component->GetClass()->GetName(), *Component->GetName());
		...
		else if (Click.GetKey() == EKeys::LeftMouseButton)
		{
			...
			if (Click.IsControlDown())
			{
				...
				if (bSelect)
				{
					UE_LOG(LGUIEditor, Log, TEXT("Clicking on Component (CTRL LMB): %s (%s)"), *Component->GetClass()->GetName(), *Component->GetName());
				}
			...
			else if (Click.IsShiftDown())
			{
				...
				{
					UE_LOG(LGUIEditor, Log, TEXT("Clicking on Component (SHIFT LMB): %s (%s)"), *Component->GetClass()->GetName(), *Component->GetName());
			...
			else
			{
				...
				UE_LOG(LGUIEditor, Log, TEXT("Clicking on Component (LMB): %s (%s)"), *Component->GetClass()->GetName(), *Component->GetName());
				GEditor->SelectComponent(Component, true, true, true);
				bComponentClicked = true;
			}
		}
	...
	void ClickSurface(FLGUIPrefabEditorViewportClient* ViewportClient,UModel* Model,int32 iSurf,const FViewportClick& Click)
	{
		...
		else if( Click.IsAltDown() && Click.GetKey() == EKeys::LeftMouseButton)
		{
			...
			if( Click.IsControlDown() )
			{
				...
				{
					UE_LOG(LGUIEditor, Log, TEXT("WARNING: the texture coordinates were not parallel to the surface.") );
				}
