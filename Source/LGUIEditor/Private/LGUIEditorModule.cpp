...
void FLGUIEditorModule::StartupModule()
{
	...
	//load these blueprint class, so PlaceActor menu can show them
	{
		//event system
		{
			FString ClassName = TEXT("LexEventSystemActor");
			LoadClass<AActor>(NULL, *FString::Printf(TEXT("/LGUI/Blueprints/%s.%s_C"), *ClassName, *ClassName));
			ClassName = TEXT("LexEventSystemActor_EnhancedInput");
			LoadClass<AActor>(NULL, *FString::Printf(TEXT("/LGUI/Blueprints/%s.%s_C"), *ClassName, *ClassName));
		}
		//prefab presenter
		for (int i = 0; i <= 3; i++)
		{
			FString ClassName;
			auto RenderMode = (ELexRenderMode)i;
			switch (RenderMode)
			{
